...
myArray3 := FeriadosMoveis(substr(Data, 7, 4), substr(DataFim, 7, 4))
if Diasemana(Data) = 0
{
...
if Aguarda < 1
{
MsgBox 0x30, ATENÇÃO, O campo tempo de resposta deve estar entre 1 e 99, corrija o erro!
}
else
{
validacao := true
...
if h_dia != 3600
{
MsgBox 0x24, Preenche REF, O horário da atividade física está diferente de 1 (uma) hora por dia. `nQuantidade de horas e minutos inseridos: %s_dia%`n`nDeseja continuar assim mesmo?
IfMsgBox No,{
Gui 1:Show
return
}
...
{
MsgBox  0x10, Preenche REF, %sURL% `n`nA página da web ativa não corresponde ao Registro de Frequência.
permiteMsg := false
Gui 1:Show
return
}
...
if (Sodesporto)
{
baseEscolhido := DesportoE . DesportoS
global time1:=A_Now
while(i < diferenca)
{
...
{
progresso := "Pulando " . corrente . " (fim de semana ou feriado - dia " . (i+1) . " de " . diferenca . ")"
Send, {Tab 4}
Send, {Space}
Espera1()
}
...
{
baseEscolhido := Entrada . Saida1 . Entrada2 . Saida2 . DesportoE . DesportoS
global time1:=A_Now
while(i < diferenca)
{
...
{
MsgBox 0x24, Preenche REF, O horário da atividade física está diferente de 1 (uma) hora por dia. `nQuantidade de horas e minutos inseridos no DIA 2: %s_dia2%`nQuantidade de horas e minutos inseridos no DIA 3: %s_dia3%`n`nDeseja continuar assim mesmo?
IfMsgBox No,{
Gui 2:Show
return
}
